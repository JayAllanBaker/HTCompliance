...
**Data Storage**: Primary database is PostgreSQL (Neon for Replit, standard for Docker) with UUIDs for primary keys. Core tables include `users`, `customers` (exposed as "organizations" in the API and UI with orgType classification), `contracts`, `compliance_items`, `billable_events`, `evidence`, `organization_notes`, `compliance_comments`, `audit_log`, `objectives`, `key_results`, and `check_ins`. Session storage is memory-based for Replit and PostgreSQL-backed for Docker. PostgreSQL enums enforce data integrity (including org_type: customer, vendor, contractor, internal, state_govt, federal_govt; confidence: green, yellow, red).
...
*   **OKR Dashboard (Measure What Matters)**: Strategic planning and tracking system that connects compliance activities to business outcomes. Features include:
    - **Auto-Calculated Metrics**: Four key metrics automatically derived from existing data - On-Time Compliance Rate, Late Fees, Average Lead Time, and Contract Coverage percentage.
    - **Objective Tracking**: Collapsible objective cards showing title, description, timeframe (Q1 FY26, Q2 FY26, etc.), owner, overall progress bar (based on average KR scoring), and confidence indicator from latest check-in.
    - **Key Results Display**: Each objective contains multiple key results with progress bars, current vs target values, baseline tracking, scoring (0-1.0 scale with color coding: red 0-0.3, yellow 0.3-0.7, green 0.7+), and unit display (percent, dollars, days, count, hours, score).
    - **Weekly Check-Ins**: Dialog interface for recording weekly progress updates including week-of date, confidence rating (green=on track, yellow=at risk, red=off track), progress notes, risks and blockers, and next week planning. Check-in history is displayed in expandable sections within objective cards.
    - **Smart Auto-Metrics**: Four key results are automatically calculated from compliance and contract data (auto_metric_type: on_time_rate, late_fees, lead_time, contract_coverage) and update in real-time as underlying data changes.
    - **Timeframe Filtering**: Dropdown filter allows users to view objectives by quarter (Q1-Q4 FY26, etc.) with active/inactive status tracking.
